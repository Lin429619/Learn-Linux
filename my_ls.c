...
void do_ls(char *dirname){
    ...
    File *fileinfo = NULL;
    fileinfo = (File *)malloc(sizeof(File) * 10000);
    ...
    while((direntp = readdir(dir_ptr)) != NULL){
        if (direntp->d_name[0] == '.' && has_a == 0)
            continue;        
        strcpy(fileinfo[File_cnt].filename, direntp->d_name);
        ...
        if(lstat(filepath, &fileinfo[File_cnt].info) == -1){
            perror(filepath);
            exit(EXIT_FAILURE);
        }
        else
...
void get_color(File fileinfo,char *name){
    ...
    if(S_ISLNK(mode)){  //符号链接
        printf("\033[01;36m%-40s\033[0m",name); //加粗深绿
    }
    else if(S_ISDIR(mode)){  //目录
        printf("\033[01;34m%-40s\033[0m",name);   //加粗蓝
    }
    else if(S_ISCHR(mode) || S_ISBLK(mode)){ //块设备和字符设备
        printf("\033[40;33m%-40s\033[0m",name);   //背景黑字体黄
    }
    else if(S_ISFIFO(mode)){  //FIFO或管道
        printf("\033[01;33m%-40s\033[0m",name);  //加粗黄
    }
    else if(S_ISSOCK(mode)){   //套接字
        printf("\033[01;35m%-40s\033[0m",name);  //加粗紫色
    }
    else if(S_ISREG(mode)){  //常规文件，并且有可执行权限
        if(mode & S_IXUSR || mode & S_IXGRP || mode & S_IXOTH){
            printf("\033[01;32m%-40s\033[0m",name);  //加粗绿
        }
        else 
            printf("%-40s",name);
    }
